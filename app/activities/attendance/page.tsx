    ...
    >
      ...
      <div className="w-full flex justify-end">
        <div className="flex mb-4 rounded-xl bg-[#181f2c] p-0.5 gap-1 text-sm">
          <button
            className={`px-3 py-1.5 rounded-xl font-medium transition-colors duration-150 focus:outline-none ${view === 'mark' ? 'bg-[#232b3b] text-white' : 'text-gray-400 hover:text-white'}`}
      ...
      {view === 'mark' ? (
        <div className="mx-auto w-full max-w-4xl">
          <AttendanceForm onSuccess={() => { setView('view'); setRefreshKey(prev => prev + 1); }} />
        </div>
      ) : (
