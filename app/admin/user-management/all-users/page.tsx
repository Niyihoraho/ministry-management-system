...
export default function AllUsersPage() {
  const [view, setView] = useState<'view' | 'add' | 'roles'>('view');
    ...
    >
      ...
      <div className="w-full flex justify-end">
        <div className="flex mb-4 rounded-xl bg-[#181f2c] p-0.5 gap-1 text-sm">
          ...
          <button
            className={`px-3 py-1.5 rounded-xl font-medium transition-colors duration-150 focus:outline-none ${view === 'roles' ? 'bg-[#232b3b] text-white' : 'text-gray-400 hover:text-white'}`}
            onClick={() => { setView('roles'); setRefreshKey(prev => prev + 1); }}
          >
            User Roles
          </button>
        </div>
      ...
      {view === 'add' ? (
        <div className="mx-auto w-full max-w-xl">
          <UserForm onSuccess={() => { setView('view'); setRefreshKey(prev => prev + 1); }} />
        </div>
      ) : view === 'roles' ? (
        <div className="mx-auto w-full max-w-7xl transition-all duration-300">
          <UserRoleTable refreshKey={refreshKey} />
        </div>
      ) : (
        <div className="mx-auto w-full max-w-7xl transition-all duration-300">
