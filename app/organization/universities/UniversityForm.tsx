...
export default function UniversityForm() {
  ...
  useEffect(() => {
    // Auto-scope region by current user's role and use RLS-filtered regions endpoint
    const init = async () => {
      try {
        ...
        // Fallback to regions list if scope fetch fails
        const regionsRes = await axios.get('/api/members/regions');
        const regions = Array.isArray(regionsRes.data) ? regionsRes.data : regionsRes.data.regions;
        if (Array.isArray(regions)) {
          setRegionOptions(regions.map((r: { id: number; name: string }) => ({ value: String(r.id), label: r.name })));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        setError(data.error || "Failed to add university.");
        setMessage(null);
      }
    } catch (err: any) {
  ...
  return (
    <ComponentCard title="Add New University">
