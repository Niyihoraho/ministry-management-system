...
import AlumniSmallGroupForm from "./AlumniSmallGroupForm";
import AlumniSmallGroupTable from "./AlumniSmallGroupTable";

export default function AlumniSmallGroupDirectoryPage() {
    ...
    >
      ...
      {view === 'add' ? (
        <div className="mx-auto w-full max-w-xl">
          <AlumniSmallGroupForm onSuccess={() => { setView('view'); setRefreshKey(prev => prev + 1); }} />
        </div>
      ) : (
        <div className="mx-auto w-full max-w-7xl transition-all duration-300">
          <AlumniSmallGroupTable refreshKey={refreshKey} />
        </div>
      )}
    </div>
  );
