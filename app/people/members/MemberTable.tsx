...
// API function to fetch all members
...
export default function MemberTable({ refreshKey }: { refreshKey?: number }) {
  ...
  const [regionMap, setRegionMap] = useState<{ [id: number]: string }>({});
  const [smallGroupMap, setSmallGroupMap] = useState<{ [id: number]: string }>({});
  const [alumniGroupMap, setAlumniGroupMap] = useState<{ [id: number]: string }>({});
  const [universityMap, setUniversityMap] = useState<{ [id: number]: string }>({});
  ...
  useEffect(() => {
    axios.get("/api/regions").then(res => {
      if (Array.isArray(res.data.regions)) {
        ...
        res.data.regions.forEach((r: any) => { map[r.id] = r.name; });
        setRegionMap(map);
      }
    });
    axios.get("/api/members/small-groups").then(res => {
      if (Array.isArray(res.data)) {
        const map: { [id: number]: string } = {};
        res.data.forEach((sg: any) => { map[sg.id] = sg.name; });
        setSmallGroupMap(map);
      }
    });
    axios.get("/api/members/alumni-small-groups").then(res => {
      if (Array.isArray(res.data)) {
        const map: { [id: number]: string } = {};
        res.data.forEach((ag: any) => { map[ag.id] = ag.name; });
        setAlumniGroupMap(map);
      }
    });
    axios.get("/api/universities").then(res => {
      if (Array.isArray(res.data.universities)) {
        const map: { [id: number]: string } = {};
        res.data.universities.forEach((u: any) => { map[u.id] = u.name; });
  ...
  const filteredMembers = useMemo(() => {
    ...
    return members.filter(member => {
      const searchableContent = [
        ...
        formatBirthPlace(member),
        member.type,
        member.status,
        member.localChurch,
        member.regionId ? regionMap[member.regionId] : '',
        member.universityId ? universityMap[member.universityId] : '',
        member.smallGroupId ? smallGroupMap[member.smallGroupId] : '',
        member.alumniGroupId ? alumniGroupMap[member.alumniGroupId] : '',
        formatDate(member.graduationDate),
        member.faculty,
  ...
  }, [members, searchQuery, regionMap, universityMap, smallGroupMap, alumniGroupMap]);
    ...
    <div className="space-y-4">
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div className="p-4 border-b border-gray-200 dark:border-white/[0.05]">
          <input
            type="text"
            placeholder="Search by name, email, phone, status, etc..."
            value={searchQuery}
        ...
        {!currentRows || currentRows.length === 0 ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-4">
                {searchQuery ? `No results for "${searchQuery}". Try a different search term.` : "There are no members to display at the moment."}
              </p>
            </div>
          </div>
        ) : (
                  ...
                  <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                    {currentRows.map((member: Member) => (
                      ...
                      >
                        ...
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400"><div><span className="block text-theme-sm">{member.email}</span><span className="block text-theme-xs text-gray-400">{member.phone}</span></div></TableCell>
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400"><div><span className="block text-theme-sm capitalize">{member.gender || "N/A"}</span><span className="block text-theme-xs text-gray-400">{formatDate(member.birthdate)}</span><span className="block text-theme-xs text-gray-400">{formatBirthPlace(member)}</span></div></TableCell>
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400"><div className="flex flex-col gap-1"><span className="capitalize text-theme-sm">{member.type}</span><Badge size="sm" color={getStatusColor(member.status)}>{getStatusDisplayName(member.status)}</Badge></div></TableCell>
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                          <div>
                            <span className="block text-theme-sm">{member.localChurch || "N/A"}</span>
                            <span className="block text-theme-xs text-gray-400">Region: {member.regionId ? regionMap[member.regionId] || `ID ${member.regionId}` : "N/A"}</span>
                            <span className="block text-theme-xs text-gray-400">Small Group: {member.smallGroupId ? smallGroupMap[member.smallGroupId] || `ID ${member.smallGroupId}` : "N/A"}</span>
                            {member.alumniGroupId && (
                              <span className="block text-theme-xs text-gray-400">Alumni Group: {alumniGroupMap[member.alumniGroupId] || `ID ${member.alumniGroupId}`}</span>
                            )}
                          </div>
                        </TableCell>
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                          <div>
                            <span className="block text-theme-sm">University: {member.universityId ? universityMap[member.universityId] || `ID ${member.universityId}` : "N/A"}</span>
                        ...
                        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400"><div><span className="block text-theme-sm capitalize">{member.maritalStatus || "N/A"}</span><span className="block text-theme-xs text-gray-400">Profession: {member.professionalism || "N/A"}</span></div></TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
