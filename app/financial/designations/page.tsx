...
export default function DesignationDirectoryPage() {
    ...
    >
      ...
      {view === 'add' ? (
        <div className="mx-auto w-full max-w-xl">
          <DesignationForm onSuccess={() => { setView('view'); setRefreshKey(prev => prev + 1); }} />
        </div>
      ) : (
        <div className="mx-auto w-full max-w-7xl transition-all duration-300">
          <DesignationTable refreshKey={refreshKey} />
        </div>
      )}
    </div>
  );
